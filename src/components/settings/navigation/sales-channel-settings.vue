    ...
    <div class="company">
        ...
        <div class="navigation-list" v-if="settings && show">
            <draggable :list="settings" v-model="settings">
                <transition-group>
                    <seller-navigation-list v-for="(element, index) in settings" :key="`seller-${index}`" :name="element.title" :subMenu="getSubMenu(element.child)" :icon="element.icon" @delete-menu="settings.splice(index, 1)" @seller-panel-show="$emit('seller-panel-show', { type: type, index: index, isEdit: true})"></seller-navigation-list>
                </transition-group>
            </draggable>
        </div>
    </div>
