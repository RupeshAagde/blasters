        ...
        <div class="main-container">
            <div class="page-container">
                <adm-jumbotron
                    ...
                    btnLabel="Create Webhook"
                    @btnClick="createWebhook"
                >
                    <nitrozen-button
                        theme="secondary"
                        v-strokeBtn
                ...
                <div @click.self="redirectEdit(items.id)" class="mirage-list-card-container">
                    ...
                    <div class="card-content-section">
                        ...
                        <div  @click.self="redirectEdit(items.id)" class="full-name card-content-line-3">
                            Events: <span @click.self="redirectEdit(items.id)"  class="webhook-span list-events">{{items.event_name.length> 3 ? items.event_name.slice(0,2).join(","):items.event_name.join(",")}}</span><a @click="selectedEvent(items)" class="event-type-count">{{items.event_name.length> 3 ?"+"+(items.event_name.length-3).toString()+"more":""}}</a>
                        </div>
                        <div @click="redirectEdit(items.id)" class="full-name card-content-line-3">
                            Modified by: <span class="webhook-span user-details">{{items.modified_by}}</span>, Modified on: <span class="webhook-span user-details">{{items.updated_on}}</span>
                        </div>
                    </div>
                    <div @click.self="redirectEdit(items.id)" class="card-badge-section">
                        ...
                        <div class="card-detail">
                            <nitrozen-button
                                @click="report(items.name)"
                                class="webhook-report-btn"
                                v-strokeBtn
                                :theme="'secondary'"
                                >View Report</nitrozen-button
                            >
                            <span class="copy" v-html="copyIcon"  @click="onCopyCode($event,items.webhook_url)" ></span>
                        </div>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        report(name){
            ...
            this.$router.push({
                path: path.join(this.$basePath, 'settings/webhook/report?page_no=1&page_size=10&start_date='+start_date+'&end_date='+end_date+'&subscriber_name='+name)
            });
        },
        selectedEvent(item){           
        ...
        reportWebhook(){
            this.$router.push({
                path: path.join(this.$basePath, 'settings/webhook/report')
            });
        },
        getSubscriberList(){
                    console.log("#####################2")
        ...
        createWebhook(){
            this.$router.push({
                path: path.join(this.$basePath, 'settings/webhook/create')
            });
        },
        redirectEdit(id){
            this.$router.push({
                path: path.join(this.$basePath, 'settings/webhook/edit/', id.toString())
            });
        },
