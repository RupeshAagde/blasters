...
describe('Category config home', () => {
    ...
    it("should check for header",()=>{
        expect(wrapper.find('.category-create-container-header').element.textContent).toBe("Add New Category")
    })
    ...
    it("should simulate error on blur",()=>{
        ...
        input.trigger('input')
        input.trigger('blur')
        expect(wrapper.vm.groupName.error).toBe(`${wrapper.vm.groupName.label} is a mandatory field`)
    })
    it("should check for the handle Save function",()=>{
        ...
        let op = {"categoryName": "test value", "selectedCategories": [123]}
