...
describe('ManifestUploadBox', () => {
    ...
    it('should set the class active on file enter', async() => {
        wrapper.vm.fileDropListner(
            {  
                preventDefault: () => {},dataTransfer:{files:[{size: 20971, type: 'application/pdf', name:'consent.pdf'}]}
            }, 
            'enter'
        );
    ...
    it('should format the file name when file name is greater than 20 characters', async()=> {
        wrapper.vm.fileDropListner(
            { 
                preventDefault: () => {},dataTransfer:{files:[{size: 20971, type: 'application/pdf', name:'consenhsldfhlhdslhdlhlhsdlhfdlhlsdhldsshlt.pdf'}]}
            }, 
            'drop'
        );
